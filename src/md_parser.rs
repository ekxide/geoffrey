...
pub enum MdParseError {
    ...
    NotAGeoffreyTag,
    #[error("Line {line_number} with text '{text}' has error: {error}!")]
    GeoffreyTagError {
        line_number: usize,
        text: String,
        error: GeoffreyTagParseError,
    },
    #[error("Line {line_number} with tag '{text}'! No code block found! Please remove the tag or blank lines between the tag and code block!")]
    GeoffreyTagWithoutCodeBlock { line_number: usize, text: String },
    #[error("Line {line_number} with tag '{text}'! Error: {error}")]
...
pub fn parse(file_content: &str) -> ParseResult<Vec<Section<'_>>> {
    let mut sections = Vec::new();
    let mut remaining_lines = file_content.lines().enumerate();
    while let Some((line_index, line)) = remaining_lines.next() {
        let section = parse_section(line_index, line, &mut remaining_lines)?;
...
fn parse_section<'a>(
    line_index: usize,
    line: &'a str,
    remaining_lines: &mut Enumerate<Lines<'a>>,
) -> ParseResult<Section<'a>> {
    let section = match geoffrey_tag_envelope(line_index, line) {
        ...
        Err(e) if e == MdParseError::NotAGeoffreyTag => Section::TextLine(line),
        Err(e) => return Err(e),
    };
...
fn md_code_block_end<'a>(
    ...
    for (n, i) in lines.by_ref() {
        ...
        let (i, code_block_fence) =
            combinator::opt(tag(backticks))
                .parse(i)
                .map_err(|_: NomError<_>| MdParseError::GeoffreyCodeBlockError {
                    line_number: n + 1,
                    text: i.into(),
    ...
    Err(MdParseError::GeoffreyCodeBlockError {
        line_number: code_block_begin_context.line_number,
...
fn md_code_block<'a>(
    ...
    let (line_index, line) = lines
        ...
        .ok_or(MdParseError::GeoffreyTagWithoutCodeBlock {
            line_number: tag_context.line_number,
            text: tag_context.text.into(),
        })?;
    let (begin, backticks) = md_code_block_begin(line).map_err(|e| match e {
...
fn geoffrey_tag_envelope(line_index: usize, line: &str) -> ParseResult<(&str, usize)> {
    let (i, spaces) = space0(line).map_err(|_: NomError<_>| MdParseError::NotAGeoffreyTag)?;
    let (i, md_comment) = sequence::delimited(tag("<!--"), is_not("-->"), tag("-->"))
        .parse(i)
...
fn geoffrey_tag_data(i: &str) -> TagParseResult<(&str, Snippet<'_>)> {
    let (i, file_name) =
...
fn geoffrey_attribute(i: &str) -> TagAttributeParseResult<(&str, &str)> {
    ...
    let (_, _) = combinator::verify(combinator::rest, |s: &str| s.is_empty())
        .parse(rem)
...
fn geoffrey_attributes_snippet_list(i: &str) -> TagParseResult<(&str, Snippet<'_>)> {
    let i = i.trim();
...
fn geoffrey_attributes_snippet_elided_block(i: &str) -> TagParseResult<(&str, Snippet<'_>)> {
    let i = i.trim();
    let (i, _) =
...
fn geoffrey_attributes_snippet_full_block(i: &str) -> TagParseResult<(&str, Snippet<'_>)> {
    match geoffrey_attribute(i) {
...
fn geoffrey_attributes_snippet_full_file(i: &str) -> TagParseResult<(&str, Snippet<'_>)> {
    let i = i.trim();
    if !i.is_empty() {
...
mod tests {
    ...
    fn valid_geoffrey_code_block() {
        let tag_line = "<!-- [geoffrey] [hypnotoad.cpp] -->";
        let mut lines = "```cpp\n```\n".lines().enumerate();
        assert_eq!(
            geoffrey_code_block(tag_line, 4, " [foo.cpp] ", &mut lines, ParseContext::default()),
            Ok(Section::GeoffreyCodeBlock {
                tag_line,
                indentation: 4,
                tag: GeoffreyTag {
        ...
        let mut lines = "```cpp\n```\n".lines().enumerate();
        assert_eq!(
            geoffrey_code_block(tag_line, 0, "[foo.cpp] [bar]", &mut lines, ParseContext::default()),
            Ok(Section::GeoffreyCodeBlock {
                tag_line,
                indentation: 0,
                tag: GeoffreyTag {
        ...
        let mut lines = "```cpp\nint maint() {}\n```\n".lines().enumerate();
        assert_eq!(
            geoffrey_code_block(tag_line, 0, "[foo.cpp]", &mut lines, ParseContext::default()),
            Ok(Section::GeoffreyCodeBlock {
                tag_line,
                indentation: 0,
                tag: GeoffreyTag {
    ...
    fn valid_parse_section() {
        let tag_line = "<!-- [geoffrey] [foo.cpp] -->";
        let mut lines = "```cpp\n```\n".lines().enumerate();
        assert_eq!(
            parse_section(0, tag_line, &mut lines),
            Ok(Section::GeoffreyCodeBlock {
                tag_line,
                indentation: 0,
                tag: GeoffreyTag {
        ...
        let tag_line = "  <!-- [geoffrey] [foo.cpp] [bar] -->";
        let mut lines = "```cpp\n```\n".lines().enumerate();
        assert_eq!(
            parse_section(0, tag_line, &mut lines),
            Ok(Section::GeoffreyCodeBlock {
        ...
        let tag_line = "<!-- [geoffrey] [foo.cpp] [[bar] [baz][bazz]] -->";
        let mut lines = "```cpp\n```\n".lines().enumerate();
        assert_eq!(
            parse_section(
                0,
